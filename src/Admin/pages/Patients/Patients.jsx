...
import { FaPlus, FaEdit, FaTrash, FaUser, FaPhone, FaEnvelope, FaHeartbeat, FaCalendarAlt, FaBirthdayCake, FaMapMarkerAlt, FaIdCard, FaUserInjured } from 'react-icons/fa';
import { MdClose, MdSearch, MdFilterList, MdMale, MdFemale, MdBloodtype } from 'react-icons/md';
...
const Patients = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'Under Treatment': return 'bg-yellow-100 text-yellow-700';
  ...
  const underTreatmentCount = patients.filter(p => p.status === 'Under Treatment').length;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 md:mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">Patients Management</h1>
          ...
          <div className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                ...
                <p className="text-xl md:text-2xl font-bold text-gray-900">{criticalCount}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <p className="text-gray-500 text-xs md:text-sm">Under Treatment</p>
                <p className="text-xl md:text-2xl font-bold text-gray-900">{underTreatmentCount}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <p className="text-gray-500 text-xs md:text-sm">Total Visits</p>
                <p className="text-xl md:text-2xl font-bold text-gray-900">{totalVisits}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
          {paginated.map(patient => (
            <div key={patient.id} className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 hover:scale-[1.02]">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className={`text-xs font-semibold px-2 md:px-3 py-1 rounded-full ${getStatusColor(patient.status)}`}>
                  {patient.status}
                </span>
              </div>
              
              <h3 className="font-bold text-base md:text-lg text-gray-900 mb-1">{patient.name}</h3>
              <p className="text-gray-600 text-xs md:text-sm mb-4">ID: {patient.id}</p>
              ...
              <div className="flex items-center gap-1 md:gap-2">
                ...
                {totalPages > 5 && (
                  <>
                    ...
                    <span className="text-gray-600 font-medium text-sm md:text-base">Page {page} of {totalPages}</span>
                  </>
                )}
              </div>
              <button 
